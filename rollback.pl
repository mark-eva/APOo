...
print color ("green"), "Please provide the patch number:\n", color("reset");
our $rollback_id = <STDIN>;
chomp $rollback_id;
...
our $proceed_patch = `$cmd_check`;
	...
	else
	{
		...
		#---Rollback Patch 
		print "Rollback patch $rollback_id on Enterprise Oracle Home\n";
                my $CMD_PATCH1 = "echo \"y\" | $ORACLE_HOME/OPatch/opatch rollback -id $rollback_id";
		print "$CMD_PATCH1 \n";
		system ($CMD_PATCH1);
...
our $proceed_patch = `$cmd_check`;
	...
	
	{
		...
		sleep (5);	
		 ...
		 my $CMD_PATCH = "echo \"y\" | $ORACLE_HOME/OPatch/opatch rollback -id $rollback_id";
		 print "$CMD_PATCH \n";
		 system ($CMD_PATCH);
